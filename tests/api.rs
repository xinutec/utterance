...
// ---- labels: ground truth a person marked -------------------------------

/// A stored take, ready to be marked up.
async fn marked_take(app: &TestApp) -> String {
    let (status, body) = upload(app, "speech", wav_fixture(2.0)).await;
    assert_eq!(status, StatusCode::OK, "{body}");
    body["meta"]["id"].as_str().expect("an id").to_string()
}

/// Replace a take's labels and return what the store kept.
async fn put_labels(app: &TestApp, id: &str, times: &[f32]) -> (StatusCode, Value) {
    let syllables: Vec<Value> = times
        .iter()
        .map(|t| serde_json::json!({ "atS": t }))
        .collect();
    let req = Request::put(format!("/api/recordings/{id}/labels"))
        .header("content-type", "application/json")
        .body(Body::from(
            serde_json::to_vec(&serde_json::json!({ "syllables": syllables })).unwrap(),
        ))
        .unwrap();
    send(app, req).await
}

async fn get_labels(app: &TestApp, id: &str) -> Value {
    let req = Request::get(format!("/api/recordings/{id}/labels"))
        .body(Body::empty())
        .unwrap();
...
/// The marked times, in the order the store returns them.
fn times(labels: &Value) -> Vec<f64> {
    labels["syllables"]
        .as_array()
        .expect("syllables")
        .iter()
        .map(|s| s["atS"].as_f64().expect("a time"))
        .collect()
}

#[tokio::test]
async fn a_recording_starts_with_nothing_marked() {
    // Not an error and not a 404. Every take made before labelling existed has
    // none, and so does every one nobody has sat down with — "not labelled" is
    // the normal state of a recording rather than a fault in it.
    let app = TestApp::new();
    let id = marked_take(&app).await;
    assert_eq!(times(&get_labels(&app, &id).await), Vec::<f64>::new());
}

#[tokio::test]
async fn marks_come_back_in_time_order_however_they_were_sent() {
    // Dragging a mark past its neighbour is ordinary, so the order is enforced
    // where they are stored rather than assumed by every reader.
    let app = TestApp::new();
    let id = marked_take(&app).await;

    let (status, stored) = put_labels(&app, &id, &[1.4, 0.2, 0.9]).await;
    assert_eq!(status, StatusCode::OK, "{stored}");
    assert_eq!(times(&stored), vec![0.2, 0.9, 1.4]);
    // And the answer is what a later reader gets, not just what was echoed.
    assert_eq!(times(&get_labels(&app, &id).await), vec![0.2, 0.9, 1.4]);
}

#[tokio::test]
async fn two_marks_closer_than_a_syllable_are_one_mark() {
    // A double tap, or a drag that landed on its neighbour. Keeping both would
    // report a rhythm nobody performed — and this is ground truth, where a wrong
    // one is worse than a missing one.
    let app = TestApp::new();
    let id = marked_take(&app).await;

    let (_, stored) = put_labels(&app, &id, &[1.0, 1.01]).await;
    assert_eq!(times(&stored).len(), 1);
}

#[tokio::test]
async fn labels_are_a_record_and_survive_the_voiceprint_being_rebuilt() {
    // **The distinction the file rests on.** A voiceprint is a pure function of
    // the audio and may be thrown away whenever `SCHEMA_VERSION` moves. Nothing
    // can re-derive this: it is somebody's ear, spent once.
    let app = TestApp::new();
    let id = marked_take(&app).await;
    put_labels(&app, &id, &[0.5]).await;

    // Fetching the detail is what brings a stale voiceprint up to date.
    let req = Request::get(format!("/api/recordings/{id}"))
        .body(Body::empty())
        .unwrap();
    let (status, body) = send(&app, req).await;
    assert_eq!(status, StatusCode::OK, "{body}");

    assert_eq!(times(&get_labels(&app, &id).await), vec![0.5]);
}

#[tokio::test]
async fn labelling_a_recording_that_is_not_here_is_refused() {
    // Labels for a take that does not exist would be unreachable and would
    // outlive it, which is a leak rather than a note.
    let app = TestApp::new();
